...
패션 브랜드 노드 위키 — 20개 스타일 클러스터 + ~2899개 브랜드를 force-directed 그래프로 시각화하고, 사용자·팀이 함께 브랜드 DB를 쌓는 **참여형 위키**로 발전 중. (현재는 **뷰 전용** — 쓰기 API는 `WIKI_WRITE_ENABLED` 게이트로 비활성화, 인증·참여 모델은 구축 예정)
...
| 그래프 시각화 | react-force-graph-2d (canvas) + 결정론적 링 배치 + fx/fy 회전 (`components/graph/GraphCanvas.tsx`) |
